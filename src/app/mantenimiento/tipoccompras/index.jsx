...
import ModalTiposComprobantesCompras from './ModalTiposComprobantesCompras';  // Importar el nuevo componente de modal

const TablaTiposComproabantesCompras = () => {
  const [tiposComprobantesCompras, setTiposComprobantesCompras] = useState([]);
  const [loading, setLoading] = useState(false);
  const [pagination, setPagination] = useState({ current: 1, pageSize: 10 });
  const [editingTipoComprobanteCompra, setEditingTipoComprobanteCompra] = useState(null);
  ...
  const fetchData = async (pagination, filters = {}, sorter = {}) => {
    ...
    try {
      ...
      setTiposComprobantesCompras(data.content);
      setPagination({
  ...
  const handleEdit = (record) => {
    setEditingTipoComprobanteCompra(record);
    setModalVisible(true);
  };
  
  // Abrir el modal de agregar
  const handleAdd = () => {
    setEditingTipoComprobanteCompra(null);
    form.resetFields();
    setModalVisible(true);
  };
  ...
  const handleSave = async (values) => {
    try {
      const monedaRequest = {
        id: editingTipoComprobanteCompra ? editingTipoComprobanteCompra.id : null,
        codigo: values.codigo,
        descripcion: values.descripcion,
        idEmpresa: sesionEmpId,
        usuarioCreacion: userCode,
        usuarioActualizacion: editingTipoComprobanteCompra ? userCode : null
      };
      
      if (editingTipoComprobanteCompra) {
        await axios.patch(`http://localhost:8080/api/compras/tipos/update/${editingTipoComprobanteCompra.id}`, monedaRequest);
        message.success('Tipo de comprobante de compra actualizado exitosamente');
      } else {
        await axios.post(`http://localhost:8080/api/compras/tipos/save`, monedaRequest);
        message.success('Tipo de comprobante de compra creado exitosamente');
      }
      ...
      if (error.response && error.response.status === 500) {
        ...
        if (errorMessage.includes("llave duplicada") && errorMessage.includes("tipos_id_empresa_codigo_key")) {
            ...
            message.error(editingTipoComprobanteCompra ? 'Error actualizando tipo de comprobante de compra' : 'Error creando tipo de comprobante de compra');
        }
      } else {
  ...
  return (
    <div className="rounded-lg dark:bg-transparent bg-transparent border shadow-md p-0 z-10 h-full relative">
      ...
      <div className="mb-2 flex justify-between border rounded-lg dark:bg-gray-800 bg-white text-gray-500 dark:text-gray-300 p-2">
        <h3 className="text-2xl font-bold">Tipos de Comprobantes de Compras</h3>
      ...
      <Table
        columns={columns}
        dataSource={tiposComprobantesCompras}
        pagination={{
          current: pagination.current,
          pageSize: pagination.pageSize,
          total: tiposComprobantesCompras.length,
      ...
      />
      {/* Modal para Moneda */}
        <ModalTiposComprobantesCompras
        visible={modalVisible}
        form={form}
        onClose={() => setModalVisible(false)}
        onSave={handleSave}
        tipoComprobanteCompra={editingTipoComprobanteCompra}
      />
    </div>
  );
...
export default TablaTiposComproabantesCompras;
