...
import ModalTiposDocumentos from './ModalTiposDocumentos';  // Importar el nuevo componente de modal

const TablaTiposDocumentos = () => {
  const [tiposDocumentos, setTiposDocumentos] = useState([]);
  const [loading, setLoading] = useState(false);
  const [pagination, setPagination] = useState({ current: 1, pageSize: 10 });
  const [editingTipoDocumento, setEditingTipoDocumento] = useState(null);
  ...
  const fetchData = async (pagination, filters = {}, sorter = {}) => {
    setLoading(true);
    try {
      const { data } = await axios.get(`http://localhost:8080/api/empresas/documentos/findAll/${sesionEmpId}`, {
        params: {
          page: pagination.current - 1,
          size: pagination.pageSize,
          docCodigo: filters.docCodigo ? filters.docCodigo[0] : null,
      ...
      setTiposDocumentos(data.content);
      setPagination({
      ...
      message.error('Error fetching tipos de documentos');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/api/empresas/documentos/delete/${id}`);
      message.success('Tipo de documento eliminado exitosamente');
      fetchData(pagination); // Refrescar la tabla
    } catch (error) {
      message.error('Error eliminando tipo de documento');
    }
  };
  
  // Abrir el modal de edición
  const handleEdit = (record) => {
    setEditingTipoDocumento(record);
  ...
  const handleSave = async (values) => {
    try {
      ...
      if (editingTipoDocumento) {
        await axios.patch(`http://localhost:8080/api/empresas/documentos/update/${editingTipoDocumento.id}`, monedaRequest);
        message.success('Tipo de documento actualizado exitosamente');
      } else {
        await axios.post(`http://localhost:8080/api/empresas/documentos/save`, monedaRequest);
      ...
      if (error.response && error.response.status === 500) {
        ...
        if (errorMessage.includes("llave duplicada") && errorMessage.includes("documentos_id_empresa_codigo_key")) {
            message.error("El código de tipo de documento ya existe para esta empresa. Por favor, elija otro código.");
        } else {
            message.error(editingTipoDocumento ? 'Error actualizando tipo de documento' : 'Error creando tipo de documento');
        }
      } else {
  ...
  const columns = [
      ...
      filterDropdown: ({
        ...
        <div style={{ padding: 8 }}>
          <Input
            ...
            onPressEnter={() => {
              confirm();
              handleSearch(selectedKeys[0], 'docCodigo');
            }}
          ...
          <Button
            ...
            onClick={() => {
              ...
              handleSearch('', 'docCodigo'); // Recarga los datos sin filtro
    ...
    },
    {
      ...
      filterDropdown: ({
        ...
        <div style={{ padding: 8 }}>
          <Input
            placeholder="Buscar código sunat"
            value={selectedKeys[0]}
            onChange={(e) => setSelectedKeys(e.target.value ? [e.target.value] : [])}
            onPressEnter={() => {
              confirm();
              handleSearch(selectedKeys[0], 'codigoSunat');
            }}
            style={{ width: 188, marginBottom: 8, display: 'block' }}
          />
          <Button
            type="link"
              onClick={() => {
                clearFilters && clearFilters();
                setSelectedKeys([]); // Limpia el estado del filtro
                handleSearch('', 'codigoSunat'); // Pasamos un valor vacío para cargar todos los datos
                confirm();
              }}
              icon={<i className="fas fa-eraser" />}
              style={{ padding: 0 }}
  ...
  return (
    <div className="rounded-lg dark:bg-transparent bg-transparent border shadow-md p-0 z-10 h-full relative">
      ...
      <div className="mb-2 flex justify-between border rounded-lg dark:bg-gray-800 bg-white text-gray-500 dark:text-gray-300 p-2">
        <h3 className="text-2xl font-bold">Tipos de Documentos</h3>
...
export default TablaTiposDocumentos;
