...
import ModalComprobanteComprasEstados from './ModalComprobanteComprasEstados';

const TablaEstadosCompras = () => {
  const [estadosCompras, setEstadosCompras] = useState([]);
  const [loading, setLoading] = useState(false);
  const [pagination, setPagination] = useState({ current: 1, pageSize: 10 });
  const [editingEstadoCompra, setEditingEstadoCompra] = useState(null);
  ...
  const fetchData = async (pagination, filters = {}, sorter = {}) => {
    setLoading(true);
    try {
      const { data } = await axios.get(`http://localhost:8080/api/compras/estados/findAll/${sesionEmpId}`, {
        params: {
      ...
      setEstadosCompras(data.content);
      setPagination({
      ...
      message.error('Error fetching estados compras');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Eliminar estado compra
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/api/compras/estados/delete/${id}`);
      message.success('Estado compra eliminado exitosamente');
      fetchData(pagination); // Refrescar la tabla
    } catch (error) {
      message.error('Error eliminando estado compra');
    }
  };
  
  // Abrir el modal de edición
  const handleEdit = (record) => {
    setEditingEstadoCompra(record);
    setModalVisible(true);
  };
  
  // Abrir el modal de agregar
  const handleAdd = () => {
    setEditingEstadoCompra(null);
    form.resetFields();
    setModalVisible(true);
  };
  ...
  const handleSave = async (values) => {
    try {
      const estadoCompraRequest = {
        id: editingEstadoCompra ? editingEstadoCompra.id : null,
        codigo: values.codigo,
        descripcion: values.descripcion,
        idEmpresa: sesionEmpId,
        usuarioCreacion: userCode,
        usuarioActualizacion: editingEstadoCompra ? userCode : null
      };
      
      if (editingEstadoCompra) {
        await axios.patch(`http://localhost:8080/api/compras/estados/update/${editingEstadoCompra.id}`, estadoCompraRequest);
        message.success('Estado compra actualizada exitosamente');
      } else {
        await axios.post(`http://localhost:8080/api/compras/estados/save`, estadoCompraRequest);
        message.success('Estado compra creada exitosamente');
      }
      ...
      if (error.response && error.response.status === 500) {
        ...
        if (errorMessage.includes("llave duplicada") && errorMessage.includes("estados_id_empresa_codigo_key")) {
          message.error("El código de estado compra ya existe para esta empresa. Por favor, elija otro código.");
        } else {
          message.error(editingEstadoCompra ? 'Error actualizando estado compra' : 'Error creando estado compra');
        }
      } else {
    ...
    },
    {
      ...
      render: (text, record) => (
        <>
          ...
          <Popconfirm
            title="¿Estás seguro de eliminar este estado compra?"
  ...
  return (
    <div className="rounded-lg dark:bg-transparent bg-transparent border shadow-md p-0 z-10 h-full relative">
      ...
      <div className="mb-2 flex justify-between border rounded-lg dark:bg-gray-800 bg-white text-gray-500 dark:text-gray-300 p-2">
        <h3 className="text-2xl font-bold">Estados compras</h3>
      ...
      <Table
        columns={columns}
        dataSource={estadosCompras}
        pagination={{
          current: pagination.current,
          pageSize: pagination.pageSize,
          total: estadosCompras.length,
        ...
        sticky
      />
      {/* Modal para Estado compra */}
      <ModalComprobanteComprasEstados
...
export default TablaEstadosCompras;
